...
static TrueStereo4chData read4chWavFile(const std::string& path, uint32_t dstSampleRate) {
    TrueStereo4chData result;
    std::string resolved = path;
    if (resolved[0] != '/') {
        std::string exeDir = getExeDir();
        if (!exeDir.empty()) {
            std::string alt = exeDir + "/" + resolved;
            struct stat st;
    ...
    if (!file) {
        std::cerr << "read4chWavFile: cannot open " << resolved << std::endl;
        return result;
    }
    char chunkID[4];
    uint32_t fileSize;
    ...
    char wave[4];
    file.read(wave, 4);
    if (std::strncmp(chunkID, "RIFF", 4) != 0 || std::strncmp(wave, "WAVE", 4) != 0) {
        std::cerr << "read4chWavFile: not a WAV file" << std::endl;
        return result;
    }
    ...
    while (file && !file.eof() && !foundData) {
        char cid[4];
        uint32_t csize;
        if (!file.read(cid, 4)) break;
        if (!file.read(reinterpret_cast<char*>(&csize), 4)) break;
        if (std::string(cid, 4) == "fmt ") {
            if (csize >= 16) {
                std::vector<char> cdata(csize);
                file.read(cdata.data(), csize);
                audioFormat = *reinterpret_cast<const uint16_t*>(cdata.data());
                numChannels = *reinterpret_cast<const uint16_t*>(cdata.data() + 2);
                fileSampleRate = *reinterpret_cast<const uint32_t*>(cdata.data() + 4);
                bitsPerSample = *reinterpret_cast<const uint16_t*>(cdata.data() + 14);
                if (audioFormat == 0xFFFE && csize >= 40) {
                    uint16_t subFormat;
                    std::memcpy(&subFormat, cdata.data() + 24, 2);
                    audioFormat = subFormat;
                }
                foundFmt = true;
            }
        } else if (std::string(cid, 4) == "data") {
            if (!foundFmt || audioFormat > 3) {
                std::cerr << "read4chWavFile: fmt not found or unsupported format" << std::endl;
                return result;
            }
            std::vector<char> rawData(csize);
            file.read(rawData.data(), csize);
            size_t bytesPerSample = bitsPerSample / 8;
            size_t totalSamples = csize / bytesPerSample;
            size_t numFrames = totalSamples / numChannels;
            if (numChannels != 4) {
                std::cerr << "read4chWavFile: expected 4 channels, got " << numChannels << std::endl;
                return result;
            }
            interleaved.resize(totalSamples);
            size_t idx = 0;
            const char* p = rawData.data();
            if (audioFormat == 3 && bitsPerSample == 32) {
                for (size_t s = 0; s < totalSamples; ++s, p += 4)
                    interleaved[s] = *reinterpret_cast<const float*>(p);
            } else if (audioFormat == 1 && bitsPerSample == 16) {
                for (size_t s = 0; s < totalSamples; ++s, p += 2)
                    interleaved[s] = static_cast<float>(*reinterpret_cast<const int16_t*>(p)) / 32768.0f;
            } else if (audioFormat == 1 && bitsPerSample == 24) {
                for (size_t s = 0; s < totalSamples; ++s, p += 3) {
                    uint32_t v = static_cast<unsigned char>(p[0]) | (static_cast<unsigned char>(p[1]) << 8) | (static_cast<unsigned char>(p[2]) << 16);
                    if (v & 0x800000) v |= 0xFF000000;
                    interleaved[s] = static_cast<float>(static_cast<int32_t>(v)) / 8388608.0f;
                }
            } else if (audioFormat == 1 && bitsPerSample == 32) {
                for (size_t s = 0; s < totalSamples; ++s, p += 4)
                    interleaved[s] = static_cast<float>(*reinterpret_cast<const int32_t*>(p)) / 2147483648.0f;
            } else {
                std::cerr << "read4chWavFile: unsupported bitsPerSample=" << bitsPerSample << std::endl;
                return result;
            }
            ...
            file.seekg(csize + (csize % 2), std::ios::cur);
        }
    }
    if (!foundData || result.ch0.empty()) {
        std::cerr << "read4chWavFile: no data found" << std::endl;
        return result;
    }
    if (dstSampleRate > 0 && fileSampleRate != dstSampleRate) {
        double srcRate = static_cast<double>(fileSampleRate);
